...
func _try_teleport(crossing_node:Dictionary) -> bool:
	...
	if node is Player:
		...
		if player.basis_correcting_tween:
			player.basis_correcting_tween.kill()
		player.basis_correcting_tween = player.create_tween()
		player.basis_correcting_tween.set_ease(Tween.EASE_IN_OUT)
		player.basis_correcting_tween.set_trans(Tween.TRANS_QUAD)
		player.basis_correcting_tween.tween_method(
			func(weight : float) -> void:
				player.global_basis = player.global_basis.slerp(player.corrected_basis().orthonormalized(), weight),
			0.,
			1.,
			5.
		)
