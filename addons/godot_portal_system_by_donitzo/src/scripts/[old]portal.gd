...
func startup() -> void:
	...
	if exit_portal == null:
		print("[DONITZO PORTALS] : WARNING portal deactivating as no exit_portal is set. name = " + name)
		visible = false
		set_process(false)
		return
	...
	material_override.shader = preload("res://addons/godot_portal_system_by_donitzo/src/shaders/portal.gdshader")
...
func _create_viewport() -> void:
	...
	_exit_camera.name = "InstantCamera"
